...
function calcNights(fechaInicio, fechaFin) {
...
export default function ResumenForm({
  ...
  const nights = calcNights(form.fechaInicio, form.fechaFin);
  const hasPresupuesto = form.presupuesto && Number(form.presupuesto) > 0;
      ...
      <div className="flex flex-col lg:flex-row gap-5 lg:items-stretch">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-neutral-1 p-6 flex-1 min-w-0 w-full">
          ...
          <SectionRow label="Nombre del viaje">
            <p className="body-bold text-primary-3">{form.nombre || '—'}</p>
          </SectionRow>
