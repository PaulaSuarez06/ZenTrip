...
function IconPeople() {
  return (
    <svg className="w-4 h-3 sm:w-5 sm:h-3.5 shrink-0" viewBox="0 0 21 14" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    ...
    <div>
      <div className="flex flex-col md:flex-row gap-5 md:items-stretch">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-neutral-1 p-6 flex-1 min-w-0 w-full">
          ...
          {/* Tab toggle */}
          <div className="flex bg-neutral-1 rounded-full p-1 gap-1 mb-5 w-full sm:w-fit">
            {TABS.map((tab) => (
              <button
                key={tab.id}
                type="button"
                onClick={() => setActiveTab(tab.id)}
                className={`flex-1 sm:flex-none flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs! sm:text-sm! font-semibold transition ${
                  activeTab === tab.id
      ...
      <div className="flex justify-between gap-3 mt-6">
        <Button variant="ghost" type="button" onClick={onAtras} className="flex-1 sm:flex-none sm:w-auto! px-4 sm:px-6 py-1.5! sm:py-2! text-xs! sm:text-sm!">
          Atrás
        </Button>
        <Button variant="orange" type="button" onClick={onSiguiente} className="flex-1 sm:flex-none sm:w-auto! px-4 sm:px-6 py-1.5! sm:py-2! text-xs! sm:text-sm!">
          Siguiente
        </Button>
      </div>
    </div>
